...
# Runtime-evaluated market dates
def get_correct_market_days(now=None):
    if now is None:
        now = datetime.utcnow()

    in_premarket = now.weekday() < 5 and (now.hour < 13 or (now.hour == 13 and now.minute < 30))
    ...
    if in_premarket and now.weekday() < 5:
        pre_market_day = now.strftime('%Y-%m-%d')
        post_market_day = days[0]
        two_days_ago = days[1]
    else:
        pre_market_day = days[0]
        post_market_day = days[0]
        two_days_ago = days[1]

    return pre_market_day, post_market_day, two_days_ago

TODAY, YESTERDAY, TWO_DAYS_AGO = get_correct_market_days()
START_DATE = (datetime.strptime(YESTERDAY, '%Y-%m-%d') - timedelta(days=30)).strftime('%Y-%m-%d')
...
async def main_async():
    async with aiohttp.ClientSession() as session:
        ...
        if "OORVOL" in df.columns and not df.empty:
